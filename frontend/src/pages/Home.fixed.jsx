...
function AlertsCampsSection({ language }) {
  ...
  // Fetch urgent requests and camps from backend
  useEffect(() => {
    const fetchData = async () => {
      try {
        setLoading(true)
        
        // Import the homepage service
        const homepageService = await import('../services/homepageService')
        
        // Fetch urgent requests
        const urgentResponse = await homepageService.getCachedUrgentRequests()
        if (urgentResponse && urgentResponse.success && urgentResponse.data.length > 0) {
          // Transform the data to match the expected format
          const transformedRequests = urgentResponse.data.map(request => ({
            title: request.title,
            hospital: request.location,
            location: request.location,
            bloodType: request.blood_type,
            units: `${request.quantity} Units`,
            time: getRelativeTime(request.created_at),
            contact: 'Call Hospital'
          }))
          setUrgentRequests(transformedRequests)
        } else {
          // Use empty array instead of default mock data
          setUrgentRequests([])
        }
        
        // Set blood camps to empty array (can be extended to fetch from backend)
        setBloodCamps([])
      } catch (err) {
        console.error('Error fetching homepage data:', err)
        // Use empty arrays instead of default mock data on error
        setUrgentRequests([])
        setBloodCamps([])
      } finally {
        setLoading(false)
          ...
          <div className="alerts-camps-grid">
            <div className="carousel-section urgent-section">
              ...
              <div className="carousel-container">
                <div className="urgent-card loading">
            ...
            <div className="carousel-section camps-section">
              ...
              <div className="carousel-container">
                <div className="camp-card loading">
                  <div className="loading-placeholder"></div>
                </div>
              </div>
            </div>
            ...
            <div className="carousel-container">
              {urgentRequests.length > 0 ? (
                ...
                <div className="no-data-message">
                  {language === 'en' ? 'No urgent requests at this time' : 'ഇപ്പോൾ അടിയന്തര അഭ്യർത്ഥനകളൊന്നുമില്ല'}
                </div>
              )}
              
              {urgentRequests.length > 1 && (
          ...
          <div className="carousel-section camps-section">
            ...
            <div className="carousel-container">
              {bloodCamps.length > 0 ? (
                ...
                <div className="no-data-message">
                  {language === 'en' ? 'No upcoming camps at this time' : 'ഇപ്പോൾ വരാനിരിക്കുന്ന ക്യാമ്പുകളൊന്നുമില്ല'}
                </div>
              )}
